        ...
        try:
            ...
            if not has_product:
                ...
                if detected_brand:
                    ...
                    current_alt_count = len(alt_docs[:2])
                    if current_alt_count < 2 and price_constraint:
                        op, limit_val = price_constraint
                        if op == 'max':
                            try:
                                import mysql.connector
                                ...
                                query = """
                                    ...
                                    LEFT JOIN hang_san_xuat hsx ON sp.ma_hang = hsx.ma_hang
                                    LEFT JOIN cau_hinh ch ON sp.ma_sp = ch.ma_sp
                                    WHERE sp.so_luong_ton > 0 AND LOWER(hsx.ten_hang) != %s AND sp.gia >= 1000000 AND sp.gia <= %s
                                    ORDER BY sp.gia ASC
                                """
                                cursor.execute(query, (detected_brand.lower(), limit_val))
                                ...
                                added_count = 0
                                for r in rows:
                                    if added_count >= (2 - current_alt_count):
                                        break
                                    # Lọc bỏ hoặc giữ phụ kiện tùy thuộc user_asked_for_accessory
                                    ...
                                    filtered_docs.append(doc)
                                    added_count += 1
                                    print(f"[RAG ALT] Thêm sản phẩm thay thế từ MySQL: {r['ten_sp']} ({price_fmt})")
                            except Exception as ex_db:
                                print(f"[RAG ALT] DB query error: {ex_db}")
                else:
                    filtered_docs.extend(product_docs_sorted[:2])
            
            price_note = ""
            if detected_brand:
                has_matching_brand_product = any(
                    (d.metadata or {}).get('type') == 'product' and (d.metadata or {}).get('ten_hang', '') == detected_brand
                    for d in filtered_docs
                )
                if not has_matching_brand_product:
                    price_note = f"\n\n⚠️ THÔNG BÁO QUAN TRỌNG: Cửa hàng HIỆN KHÔNG CÓ sản phẩm nào của hãng {detected_brand} trong tầm giá phù hợp với yêu cầu của khách hàng. Bạn BẮT BUỘC phải bắt đầu câu trả lời bằng việc khẳng định rõ ràng và lịch sự điều này (Ví dụ: 'Dạ, hiện tại dòng máy của hãng {detected_brand} trong tầm giá này bên em đang tạm hết hàng ạ'). Sau đó, giới thiệu các sản phẩm hãng khác dưới đây để khách tham khảo. TUYỆT ĐỐI KHÔNG TỰ BỊA ĐẶT sản phẩm {detected_brand} có giá này."
...
Trả lời lịch sự, thân thiện bằng tiếng Việt (HTML thuần, KHÔNG dùng markdown):"""
  ...
  Ảnh: {p['anh_dai_dien'] or ''}"""
        ...
        if not db_products and brand:
            ...
            price_note = f"\n\n⚠️ THÔNG BÁO QUAN TRỌNG: Cửa hàng HIỆN KHÔNG CÓ bất kỳ sản phẩm {product_type_label} nào của hãng {brand} trong tầm giá/ngân sách phù hợp yêu cầu. Bạn BẮT BUỘC phải bắt đầu câu trả lời bằng cách khẳng định rõ ràng và lịch sự là cửa hàng không có sản phẩm {brand} trong phân khúc này (Ví dụ: 'Dạ, hiện tại dòng {product_type_label} của hãng {brand} ở tầm giá này bên em đang tạm hết hàng ạ'). Sau đó, gợi ý khách tham khảo các máy của hãng khác đang có sẵn như bên dưới. TUYỆT ĐỐI KHÔNG TỰ BỊA SẢN PHẨM."
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    LEFT JOIN hang_san_xuat hsx ON sp.ma_hang = hsx.ma_hang
                    LEFT JOIN cau_hinh ch ON sp.ma_sp = ch.ma_sp
                    WHERE sp.so_luong_ton > 0 AND (LOWER(hsx.ten_hang) != %s OR hsx.ten_hang IS NULL)
                    ORDER BY sp.gia ASC LIMIT 30
                """, (brand.lower() if brand else '',))
                raw_alts = cursor.fetchall()
                cursor.close()
                conn.close()
                
                for p in raw_alts:
                    is_acc = is_accessory(p['ten_sp'])
                    if user_asked_for_accessory and is_acc:
                        alternatives.append(p)
                    elif not user_asked_for_accessory and not is_acc:
                        alternatives.append(p)
                    if len(alternatives) >= 3:
                        break
            except Exception as e:
                print(f"[Alternatives Search] DB error: {e}")
                print(f"[Alternatives Search] DB error: {e}")
...
Yêu cầu: Hãy tạo lại câu trả lời và KHÔNG ĐƯỢC lặp lại lỗi trên. Hãy bám sát 100% dữ liệu chuẩn và tuân thủ các quy tắc đã nêu ở trên."""
    ...
    def rewrite_query(self, current_input: str, history: List[Dict] = None) -> str:
        ...
        prompt_text = f"""Dựa vào lịch sử hội thoại dưới đây, hãy viết lại câu hỏi cuối cùng của người dùng thành một câu hỏi hoặc yêu cầu độc lập, đầy đủ ngữ cảnh (bao gồm tên hãng, dòng điện thoại/phụ kiện, khoảng giá, màu sắc, cấu hình, v.v. được đề cập trước đó).
Yêu cầu:
